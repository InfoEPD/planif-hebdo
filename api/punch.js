...
// board Monday "⏱️ Poinçons". Nécessite un jeton de session employé (voir action 'login'
// ci-dessous et api/_auth/employeeAuth.js — authentification maison téléphone + mot de passe,
// EPD uniquement ; ne concerne PAS les comptes admin/bureau ni les tenants, qui restent sur
// Clerk — voir le dispatcher verifyCaller() dans le handler plus bas).
//
...
// Clerk reste utilisé ICI uniquement pour les comptes admin/bureau (ressources Loi 25
// 'settings'/'incidents'/'dataRequests' — voir isAdminRole/isAdminOrReadOnly plus bas). Les
// comptes EMPLOYÉS (mobile) sont authentifiés via le jeton maison employeeAuth.js depuis
// l'abandon de Clerk pour ce volet — voir verifyCaller() dans le handler.
const { verifyToken } = require('@clerk/backend');
const { verifyEmployeeToken, hashPassword, verifyPassword, createEmployeeToken, toE164 } = require('./_auth/employeeAuth');
...
// Utilisés par l'action 'login' (authentification maison téléphone + mot de passe) — mêmes IDs
// que admin.html/employee-admin.js. EMP_ENTREPRISE_COL/EMP_STATUT_ACTIF_COL/EMP_STATUT_ACTIF_VAL
// reproduisent exactement les règles de loadDraftRecipientCandidates() (planif.html) : seuls les
// employés EPD au statut "Actif" peuvent se connecter.
const EMP_PHONE_COL = 't_l_phone_mkmx24h2';
const COL_ACCESS = 'boolean_mm6dd03t'; // "Accès mobile" — coché par employee-admin.js à la création du compte
const EMP_ENTREPRISE_COL = 'color_mkpfwcat';
const EMP_ENTREPRISE_EPD_VAL = 1;
const EMP_STATUT_ACTIF_COL = 'statut__1';
const EMP_STATUT_ACTIF_VAL = 1;

// Normalise un numéro de téléphone en 10 chiffres (même convention que formatPhone10() côté
// admin.html) pour comparer un numéro soumis au login avec la valeur texte libre stockée dans la
// colonne Monday EMP_PHONE_COL, peu importe son format d'origine (espaces, tirets, indicatif +1).
...
// Authentifie l'appelant, Clerk (admin/bureau) OU jeton employé maison (mobile) — voir
// employeeAuth.js. Les deux formats se distinguent trivialement par leur nombre de segments :
// un JWT Clerk a toujours 3 parties (header.payload.signature) séparées par un point, alors que
// notre jeton maison n'en a que 2 (corps.signature) — voir createEmployeeToken(). Jette une
// erreur dans les deux cas si invalide/expiré (même contrat que verifyToken() de Clerk), pour que
// le reste du handler n'ait pas à savoir lequel des deux a été utilisé.
async function verifyCaller(bearerToken, secretKey) {
  const parts = String(bearerToken || '').split('.');
  if (parts.length === 3) return await verifyToken(bearerToken, { secretKey });
  return verifyEmployeeToken(bearerToken);
}

// Authentification employé (téléphone + mot de passe) — remplace l'ancien flux Clerk pour
// l'interface mobile EPD (voir migration-employee-credentials.sql / db/schema.js
// employeeCredentials). Aucun jeton requis en entrée : c'est ainsi qu'on en obtient un.
async function handleEmployeeLogin(req, res, mondayToken) {
  const { phone, password } = req.body || {};
  if (!phone || !password) { res.status(400).json({ error: 'Numéro de téléphone et mot de passe requis.' }); return; }
  const phone10 = phoneDigits10(phone);
  if (phone10.length !== 10) { res.status(400).json({ error: 'Numéro de téléphone invalide.' }); return; }

  // Message générique volontaire (téléphone OU mot de passe incorrect OU compte désactivé) —
  // ne jamais révéler laquelle des trois conditions a échoué (évite l'énumération de comptes).
  const genericError = () => res.status(401).json({ error: 'Numéro de téléphone ou mot de passe incorrect.' });

  const data = await mondayGraphQL(mondayToken, `
    query($board: [ID!], $queryParams: ItemsQuery) {
      boards(ids: $board) {
        items_page(query_params: $queryParams, limit: 200) {
          items { id name column_values(ids: ["${EMP_PHONE_COL}", "${COL_ACCESS}"]) { id text } }
        }
      }
    }
  `, {
    ...
    queryParams: {
      rules: [
        { column_id: EMP_ENTREPRISE_COL, compare_value: [EMP_ENTREPRISE_EPD_VAL], operator: 'any_of' },
        { column_id: EMP_STATUT_ACTIF_COL, compare_value: [EMP_STATUT_ACTIF_VAL], operator: 'any_of' }
      ]
    }
  });
  ...
  const match = items.find(it => {
    ...
    const access = cv[COL_ACCESS] && cv[COL_ACCESS].text === 'v';
    return access && phoneDigits10((cv[EMP_PHONE_COL] || {}).text) === phone10;
  });
  if (!match) { genericError(); return; }

  const employeeItemId = String(match.id);
  const db = getDb();
  const [cred] = await db.select().from(schema.employeeCredentials)
    .where(eq(schema.employeeCredentials.employeeItemId, employeeItemId));
  if (!cred || !verifyPassword(password, cred.passwordHash)) { genericError(); return; }

  const token = createEmployeeToken({ role: 'employee', employeeItemId, employeeName: match.name || '' });
  res.status(200).json({
    token,
    employeeItemId,
...
module.exports = async function handler(req, res) {
  ...
  // Connexion employé (téléphone + mot de passe) — aucun jeton en entrée, voir plus haut.
  if (req.body && req.body.action === 'login') {
    try {
      await handleEmployeeLogin(req, res, mondayToken);
    } catch (err) {
  ...
  let claims;
  try {
    claims = await verifyCaller(bearerToken, secretKey);
  } catch (err) {
  ...
  try {
    // Changement de mot de passe (self-service) — utilisé au premier login (mustChangePassword)
    // et volontairement par l'employé ensuite. Remplace l'ancien flux Clerk signIn.resetPassword().
    if (action === 'changePassword') {
      const { newPassword } = req.body || {};
      if (!newPassword || String(newPassword).length < 5) {
        res.status(400).json({ error: 'Le mot de passe doit contenir au moins 5 caractères.' });
        return;
      }
      const db = getDb();
      const passwordHash = hashPassword(newPassword);
      await db.update(schema.employeeCredentials)
        .set({ passwordHash, mustChangePassword: false, updatedAt: new Date(), updatedBy: `employee:${employeeItemId}` })
        .where(eq(schema.employeeCredentials.employeeItemId, employeeItemId));
      res.status(200).json({ ok: true });
      return;
    }
