...
// Actions (POST { action, ... }) :
//   list              {}
//   create            { name, slug, ccqEnabled, primesEnabled, payrollSoftware }
//   updateSettings    { tenantId, ccqEnabled, primesEnabled, payrollSoftware }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'create') {
      const { name, ccqEnabled, primesEnabled, payrollSoftware } = req.body || {};
      let { slug } = req.body || {};
    ...
    if (action === 'updateSettings') {
      const { tenantId, ccqEnabled, primesEnabled, payrollSoftware } = req.body || {};
      ...
      await logAudit(db, ctx, 'update_settings', tenantId, { ccqEnabled, primesEnabled, payrollSoftware });
