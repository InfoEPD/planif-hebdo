...
//   listAuditLog      { limit }                              // journal des actions superuser (200 par défaut)
//
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'listAuditLog') {
      // Journal des actions superuser (créations/suspensions de tenant, réglages, premier accès
      // admin) — écrit par logAudit() ci-dessus à chaque action de ce fichier. Lecture seule,
      // aucun rôle autre que superuser n'y a jamais accès (voir requireSuperuserContext).
      const { limit } = req.body || {};
      const cappedLimit = Math.max(1, Math.min(500, Number(limit) || 200));
      const rows = await db.select().from(schema.superuserAuditLog)
        .orderBy(desc(schema.superuserAuditLog.createdAt))
        .limit(cappedLimit);
      const tenantIds = [...new Set(rows.map(r => r.tenantId).filter(Boolean))];
      const tenants = tenantIds.length ? await db.select().from(schema.tenants) : [];
      const tenantMap = Object.fromEntries(tenants.map(t => [t.id, t]));
      const entries = rows.map(r => ({
        id: r.id,
        actorLabel: r.actorLabel || r.actorClerkUserId,
        action: r.action,
        tenantId: r.tenantId,
        tenantName: r.tenantId ? ((tenantMap[r.tenantId] || {}).name || '(supprimé)') : null,
