    ...
    if (resource === 'punches') {
      ...
      if (action === 'listForApproval') {
        ...
        colleagueRows.forEach(r => {
          ...
          const hours = computePunchHours(r, pauseConfig);
          // Contrairement à EPD, les cases de pause sont gérées PAR POINÇON (segment) côté tenant
          // (voir computePunchHours ci-dessus) — pas au niveau de la journée entière. Le détail
          // affiché à l'Approbateur reflète donc directement ce poinçon, avec les mêmes seuils
          // 13h/14h (basés sur l'heure de fin) que computePunchHours.
          const finishMinutes = r.clockOut ? torontoMinutesOfDay(new Date(r.clockOut)) : -1;
          const punchOut = {
            itemId: r.id,
            ...
            breakMorning: !!r.breakMorning,
            breakLunch: !!r.breakLunch,
            breakAfternoon: !!r.breakAfternoon,
            lunchApplicable: finishMinutes >= 13 * 60,
            afternoonApplicable: finishMinutes >= 14 * 60
          };
