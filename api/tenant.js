...
//     action: recordGpsConsent {}                                      // Loi 25 — acquitte le consentement GPS (écran "Mes données")
//     action: myDataExport    {}                                       // Loi 25 — export complet des données personnelles de l'employé
...
//
//   resource: 'settings'      — Loi 25, conservation (fusionné depuis l'ancien api/privacy.js,
//                                pour rester sous la limite de 12 fonctions Serverless Hobby —
//                                voir aussi api/punch.js pour l'équivalent côté EPD)
//     action: get      {}                                    // admin/lecture-seule
//     action: update   { retentionGpsDays, retentionPunchYears }   // admin
//
//   resource: 'incidents'     — Loi 25, registre des incidents de confidentialité
//     action: list     {}                                    // admin/lecture-seule
//     action: create   { description, occurredAt, personsAffectedCount, severity, containmentActions }  // admin
//     action: update   { id, patch: { severity, containmentActions, reportedToCai, reportedToCaiAt,
//                                      notifiedPersons, notifiedPersonsAt } }                             // admin
//
//   resource: 'dataRequests'  — Loi 25, demandes d'accès/rectification/suppression/retrait
//     action: create    { requesterName, requesterContact, type, details }  // employé (employeeRef
//                        forcé à son propre id) OU admin/lecture-seule
//     action: listAll   {}                                                 // admin/lecture-seule
//     action: update    { id, patch: { status, adminNote } }               // admin

const { eq, and, ne, inArray, desc } = require('drizzle-orm');
...
// dîner/la pause PM sont applicables (voir computeDayGroup ci-dessous).
...
// Calcule les heures BRUTES d'UN poinçon fermé (clockIn/clockOut tous deux définis), sans aucun
// ajustement de pause — l'ajustement des pauses se fait désormais au niveau de la JOURNÉE (voir
// computeDayGroup()/computeDayAdjustedHours() ci-dessous, même convention que api/punch.js/
// admin.html côté EPD). Conservée telle quelle comme utilitaire bas niveau (brut seulement) ;
// n'est plus utilisée pour calculer l'Ajusté d'un poinçon — voir computeDayGroup() pour ça.
function computePunchBrutH(punch) {
  if (!punch.clockIn || !punch.clockOut) return null;
  const rawMin = Math.max(0, (new Date(punch.clockOut).getTime() - new Date(punch.clockIn).getTime()) / 60000);
  return Math.round((round15Min(rawMin) / 60) * 100) / 100;
}

// Calcule les totaux BRUT/AJUSTÉ d'UNE JOURNÉE complète pour UN employé (un ou plusieurs
// poinçons/segments — ex. en cas de changement de chantier en cours de journée) et détermine quel
// segment (celui qui a travaillé le PLUS D'HEURES ce jour-là, pas nécessairement le dernier fermé
// chronologiquement) absorbe l'ajustement des 3 pauses — même convention que EPD (voir
// computeDayPayable()/saveDayGroup() dans admin.html, et l'action 'finish' de api/punch.js). Les
// segments NON-biggest sont payés = leur propre brut (aucun ajustement, comme un "switch"), peu
// importe la valeur de leurs propres cases breakMorning/breakLunch/breakAfternoon (ignorées) —
// seules celles du segment biggest sont significatives. `dayPunches` doit contenir tous les
// poinçons d'UN SEUL employé pour UNE SEULE date (fuseau Toronto), sinon le résultat n'a pas de
// sens. Retourne { hoursByPunchId: Map(punchId -> {brutH, ajusteH}), summary: { lunchApplicable,
// afternoonApplicable, breakMorning, breakLunch, breakAfternoon, biggestPunchId } | null } —
// summary est null si aucun segment complet (clockIn ET clockOut) n'existe ce jour-là.
function computeDayGroup(dayPunches, pauseConfig) {
  const pc = pauseConfig || { matin: 15, diner: 30, pm: 15 };
  const hoursByPunchId = new Map();
  dayPunches.forEach(p => {
    if (!p.clockIn || !p.clockOut) hoursByPunchId.set(p.id, { brutH: null, ajusteH: null });
  });

  // Seuls les segments COMPLETS comptent pour choisir le "biggest" et pour déterminer l'heure de
  // fin de la journée — un segment encore ouvert ne doit ni influencer ces calculs, ni être choisi
  // comme biggest (durée réelle inconnue tant qu'il n'est pas fermé).
  const complete = dayPunches
    .filter(p => p.clockIn && p.clockOut)
    .map(p => {
      const rawMin = Math.max(0, (new Date(p.clockOut).getTime() - new Date(p.clockIn).getTime()) / 60000);
      return { punch: p, rawMin, brutMin: round15Min(rawMin), finishMin: torontoMinutesOfDay(new Date(p.clockOut)) };
    })
    .sort((a, b) => new Date(a.punch.clockIn) - new Date(b.punch.clockIn));
  if (!complete.length) return { hoursByPunchId, summary: null };

  // Seuil dîner/pause PM basé sur l'heure de fin la PLUS TARDIVE de tous les segments du jour
  // (pas celle du segment individuel) — même convention que EPD.
  const dayFinishMin = complete.reduce((max, c) => Math.max(max, c.finishMin), 0);
  const lunchApplicable = dayFinishMin >= 13 * 60;
  const afternoonApplicable = dayFinishMin >= 14 * 60;

  // Le segment le plus long absorbe l'ajustement des pauses — à égalité, le dernier segment
  // chronologique gagne (comparaison ">=" en parcourant les segments triés par heure de début
  // croissante, même convention que admin.html/computeDayPayable).
  let biggest = complete[0];
  complete.forEach(c => { if (c.brutMin >= biggest.brutMin) biggest = c; });

  const morningSkipped = !biggest.punch.breakMorning;
  const lunchSkipped = lunchApplicable && !biggest.punch.breakLunch;
  const afternoonSkipped = afternoonApplicable && !biggest.punch.breakAfternoon;

  const sumRawMin = complete.reduce((s, c) => s + c.rawMin, 0);
  const lunchAdjust = lunchApplicable ? (lunchSkipped ? 0 : -pc.diner) : 0;
  const dayPayableRaw = sumRawMin + lunchAdjust + (morningSkipped ? pc.matin : 0) + (afternoonSkipped ? pc.pm : 0);
  const dayPayableMin = Math.max(0, round15Min(dayPayableRaw));

  const otherSum = complete.reduce((s, c) => c === biggest ? s : s + c.brutMin, 0);
  const biggestAjusteMin = Math.max(0, dayPayableMin - otherSum);

  complete.forEach(c => {
    const ajusteMin = c === biggest ? biggestAjusteMin : c.brutMin;
    hoursByPunchId.set(c.punch.id, {
      brutH: Math.round((c.brutMin / 60) * 100) / 100,
  ...
  return {
    ...
    summary: {
      ...
      breakMorning: !!biggest.punch.breakMorning,
      breakLunch: !!biggest.punch.breakLunch,
      breakAfternoon: !!biggest.punch.breakAfternoon,
      biggestPunchId: biggest.punch.id,
    },
  };
}

// Regroupe une liste de poinçons (un ou plusieurs employés, un ou plusieurs jours — ex. la semaine
// entière) par employé+jour (clé "employeeId|YYYY-MM-DD", fuseau Toronto) et calcule, pour chaque
// groupe, les totaux "par jour" (voir computeDayGroup ci-dessus). Retourne { hoursByPunchId,
// summaryByKey } — hoursByPunchId couvre TOUS les poinçons fournis (clé = punch.id) ; summaryByKey
// est indexée par la même clé "employeeId|YYYY-MM-DD" (une seule fois par jour, absente si aucun
// segment complet ce jour-là).
function computeDayAdjustedHours(punches, pauseConfig) {
  const byKey = new Map();
  punches.forEach(p => {
    if (!p.clockIn) return;
    const key = `${p.employeeId}|${torontoDateKey(new Date(p.clockIn))}`;
    if (!byKey.has(key)) byKey.set(key, []);
    byKey.get(key).push(p);
  });
  const hoursByPunchId = new Map();
  const summaryByKey = new Map();
  byKey.forEach((dayPunches, key) => {
    const { hoursByPunchId: h, summary } = computeDayGroup(dayPunches, pauseConfig);
    h.forEach((v, k) => hoursByPunchId.set(k, v));
    if (summary) summaryByKey.set(key, summary);
  });
  return { hoursByPunchId, summaryByKey };
}
...
// Nom affiché de la personne qui approuve/rejette une feuille de temps depuis le portail Admin
// (mon-entreprise.html) — affiché ensuite à côté du poinçon (punches.approvedByName), exactement
// comme pour l'Approbateur mobile (voir resource 'punches'/action 'submitApproval'). Retombe sur
// l'email, puis sur 'Admin', si le nom Clerk n'est pas disponible — ne bloque jamais l'action.
async function getCallerDisplayName(ctx) {
  try {
    const secretKey = process.env.CLERK_SECRET_KEY;
    if (!secretKey || !ctx.callerId) return 'Admin';
    const clerk = createClerkClient({ secretKey });
    const user = await clerk.users.getUser(ctx.callerId);
    const fullName = [user.firstName, user.lastName].filter(Boolean).join(' ').trim();
    if (fullName) return fullName;
    const email = (user.emailAddresses || []).find(e => e.id === user.primaryEmailAddressId) || (user.emailAddresses || [])[0];
    return (email && email.emailAddress) || 'Admin';
  } catch (err) {
    return 'Admin';
  }
}
...
module.exports = async function handler(req, res) {
  ...
  // une seule fois par requête et passées à computeDayGroup()/computeDayAdjustedHours() partout où
  // c'est nécessaire. Défauts historiques (15/30/15) si la ligne tenant_settings n'a pas encore ces
  // colonnes remplies.
  ...
  try {
    ...
    if (resource === 'employees') {
      ...
      if (action === 'create') {
        const { fullName, phone, email, jobTitle, employeeNumber, primeId, address, homeLat, homeLng } = req.body || {};
        ...
        const [row] = await db.insert(schema.employees).values({
          ...
          homeLat: address ? homeLat : null,
          homeLng: address ? homeLng : null,
        }).returning();
        res.status(200).json({ ok: true, employee: row });
        return;
      }
      if (action === 'update') {
        const { id, fullName, phone, email, jobTitle, employeeNumber, status, address, homeLat, homeLng, excludeFromPayroll, primeId, isApprover } = req.body || {};
        ...
        // Voir employees.isApprover — affiche le bouton mobile "Approbation" (resource 'punches',
        // actions 'listForApproval'/'submitApproval').
        if (isApprover !== undefined) patch.isApprover = !!isApprover;
    ...
    if (resource === 'punches') {
      ...
      if (action === 'activeProjects') {
        ...
        // isApprover : piggybacké ici (premier appel fait par mon-poincon.html au démarrage) pour
        // éviter un aller-retour dédié — affiche le bouton mobile "Approbation" le cas échéant.
        // gpsConsentAt (Loi 25) est aussi piggybacké ici — mon-poincon.html l'utilise pour savoir
        // s'il doit afficher la modale de consentement GPS avant d'autoriser le tout premier poinçon.
        res.status(200).json({ projects: rows, isApprover: !!me.isApprover, gpsConsentAt: me.gpsConsentAt });
        return;
      }
      ...
      // mon-poincon.html avant le tout premier poinçon. Journalisé sur employees.gpsConsentAt
      // (voir db/schema.js) ; ré-appelable sans effet de bord (écrase simplement l'horodatage).
      if (action === 'recordGpsConsent') {
        await db.update(schema.employees).set({ gpsConsentAt: new Date() })
      ...
      // renseignements personnels détenus par Exacto (profil, poinçons, messages). Utilisé par
      // l'écran "Mes données" de mon-poincon.html.
      if (action === 'myDataExport') {
        const myPunches = await db.select().from(schema.punches)
          .where(and(eq(schema.punches.tenantId, tenantId), eq(schema.punches.employeeId, employeeId)));
        const myMessages = await db.select().from(schema.tenantMessages)
          .where(and(eq(schema.tenantMessages.tenantId, tenantId), eq(schema.tenantMessages.employeeId, employeeId)));
        res.status(200).json({
          exportedAt: new Date().toISOString(),
          profile: {
            fullName: me.fullName, phone: me.phone, email: me.email, jobTitle: me.jobTitle,
            employeeNumber: me.employeeNumber, address: me.address, status: me.status,
            gpsConsentAt: me.gpsConsentAt, createdAt: me.createdAt,
          },
          punches: myPunches.map(p => ({
            clockIn: p.clockIn, clockOut: p.clockOut, projectId: p.projectId, tache: p.tache,
            status: p.status, gpsLatIn: p.gpsLatIn, gpsLngIn: p.gpsLngIn,
            gpsLatOut: p.gpsLatOut, gpsLngOut: p.gpsLngOut, kmTraveled: p.kmTraveled,
          })),
          messages: myMessages.map(m => ({ author: m.author, body: m.body, createdAt: m.createdAt })),
        });
        return;
      }
      ...
      if (action === 'finish') {
        ...
        // Recalcule au niveau de la JOURNÉE (voir computeDayGroup) : les pauses cochées ici ne
        // seront réellement appliquées que si ce poinçon s'avère être le segment le plus long de
        // la journée de l'employé — sinon il est payé neutre (= son propre brut), même convention
        // que EPD.
        const dateKeyFinish = torontoDateKey(new Date(row.clockIn));
        const dayRowsFinish = await db.select().from(schema.punches)
          .where(and(eq(schema.punches.tenantId, tenantId), eq(schema.punches.employeeId, employeeId)));
        const sameDayFinish = dayRowsFinish.filter(r => r.clockIn && torontoDateKey(new Date(r.clockIn)) === dateKeyFinish);
        const { hoursByPunchId: hoursFinish } = computeDayAdjustedHours(sameDayFinish, pauseConfig);
        res.status(200).json({ punch: row, ...(hoursFinish.get(row.id) || { brutH: null, ajusteH: null }) });
        return;
      }
      ...
      if (action === 'editPending') {
        ...
        const pauseFieldsSent = (breakMorning !== undefined) || (breakLunch !== undefined) || (breakAfternoon !== undefined);
        if (pauseFieldsSent) {
          // Les cases de pause n'ont de sens payable que pour une journée à UN SEUL segment (aucun
          // changement de chantier en cours de route) — sur une journée à plusieurs segments, la
          // répartition des pauses entre segments (lequel est "biggest") reste réservée à l'admin.
          // Même contrôle que api/punch.js (EPD, action 'editPending').
          const dateKeyPending = torontoDateKey(new Date(punch.clockIn));
          const empDayRowsPending = await db.select().from(schema.punches)
            .where(and(eq(schema.punches.tenantId, tenantId), eq(schema.punches.employeeId, employeeId)));
          const sameDayCountPending = empDayRowsPending.filter(r => r.clockIn && torontoDateKey(new Date(r.clockIn)) === dateKeyPending).length;
          if (sameDayCountPending > 1) {
        ...
        const dateKeyPendingRow = torontoDateKey(new Date(row.clockIn));
        const dayRowsPending = await db.select().from(schema.punches)
          .where(and(eq(schema.punches.tenantId, tenantId), eq(schema.punches.employeeId, employeeId)));
        const sameDayPending = dayRowsPending.filter(r => r.clockIn && torontoDateKey(new Date(r.clockIn)) === dateKeyPendingRow);
        const { hoursByPunchId: hoursPending } = computeDayAdjustedHours(sameDayPending, pauseConfig);
        res.status(200).json({ punch: row, ...(hoursPending.get(row.id) || { brutH: null, ajusteH: null }) });
        return;
      }
      ...
      if (action === 'listToday') {
        ...
        const { hoursByPunchId: hoursToday } = computeDayAdjustedHours(todays, pauseConfig);
        res.status(200).json({ punches: todays.map(r => ({ ...r, ...(hoursToday.get(r.id) || { brutH: null, ajusteH: null }) })) });
        return;
      }
      ...
      if (action === 'listHistory') {
        ...
        const { hoursByPunchId: hoursHistory } = computeDayAdjustedHours(weekRows, pauseConfig);
        res.status(200).json({
          weekStart: wk,
          punches: weekRows
            .sort((a, b) => new Date(a.clockIn) - new Date(b.clockIn))
            .map(r => ({ ...r, ...(hoursHistory.get(r.id) || { brutH: null, ajusteH: null }), projectName: (projectMap[r.projectId] || {}).name || null })),
        });
        return;
      }
      ...
      if (action === 'listForApproval') {
        ...
        // LUI-MÊME travaillé — voir employees.isApprover. Même semaine clé (dimanche) que le reste
        // de l'interface mobile (voir action 'listHistory' ci-dessus).
        if (!me.isApprover) {
        ...
        const wk = weekStart || sundayOfWeek(torontoDateKey(new Date()));
        const nextWk = addDays(wk, 7);
        const allRows = await db.select().from(schema.punches).where(eq(schema.punches.tenantId, tenantId));
        const weekRows = allRows.filter(r => r.clockIn && torontoDateKey(new Date(r.clockIn)) >= wk && torontoDateKey(new Date(r.clockIn)) < nextWk);

        const ownPairs = new Set(
          weekRows.filter(r => r.employeeId === employeeId)
            .map(r => `${torontoDateKey(new Date(r.clockIn))}|${r.projectId}`)
        );
        const colleagueRows = weekRows.filter(r =>
          r.employeeId !== employeeId && ownPairs.has(`${torontoDateKey(new Date(r.clockIn))}|${r.projectId}`)
        );

        const empIds = [...new Set(colleagueRows.map(r => r.employeeId))];
        const projIds = [...new Set(colleagueRows.map(r => r.projectId).filter(Boolean))];
        const emps = empIds.length ? await db.select().from(schema.employees).where(eq(schema.employees.tenantId, tenantId)) : [];
        const projects = projIds.length ? await db.select().from(schema.projects).where(eq(schema.projects.tenantId, tenantId)) : [];
        const empMap = Object.fromEntries(emps.map(e => [e.id, e]));
        const projMap = Object.fromEntries(projects.map(p => [p.id, p]));

        // Calcul PAR JOUR (voir computeDayAdjustedHours) sur l'ENSEMBLE des poinçons de la semaine
        // (pas seulement colleagueRows) : le segment "biggest" d'un collègue peut se trouver sur un
        // chantier différent de celui affiché ici à l'Approbateur — même convention que EPD (voir
        // dayPauseByEmpDate dans api/punch.js).
        const { hoursByPunchId: hoursApproval, summaryByKey: summaryApproval } = computeDayAdjustedHours(weekRows, pauseConfig);

        const byDate = {};
        colleagueRows.forEach(r => {
          const date = torontoDateKey(new Date(r.clockIn));
          const proj = projMap[r.projectId];
          const hours = hoursApproval.get(r.id) || { brutH: null, ajusteH: null };
          // Le détail des pauses affiché vient du segment le plus long de la journée de CE collègue
          // (summaryApproval), pas du poinçon précis affiché ici — même convention qu'EPD.
          const daySummary = summaryApproval.get(`${r.employeeId}|${date}`) ||
            { lunchApplicable: false, afternoonApplicable: false, breakMorning: false, breakLunch: false, breakAfternoon: false };
          const punchOut = {
            itemId: r.id,
            empName: (empMap[r.employeeId] || {}).fullName || '?',
            heureDebut: r.clockIn ? new Date(r.clockIn).toLocaleTimeString('fr-CA', { hour: '2-digit', minute: '2-digit' }) : '',
            heureFin: r.clockOut ? new Date(r.clockOut).toLocaleTimeString('fr-CA', { hour: '2-digit', minute: '2-digit' }) : '',
            statut: r.status, tache: r.tache || '',
            totalAjuste: hours.ajusteH,
            approvedByName: r.approvedByName || '', rejectionComment: r.rejectionComment || '',
            breakMorning: daySummary.breakMorning,
            breakLunch: daySummary.breakLunch,
            breakAfternoon: daySummary.breakAfternoon,
            lunchApplicable: daySummary.lunchApplicable,
            afternoonApplicable: daySummary.afternoonApplicable
          };
          if (!byDate[date]) byDate[date] = {};
          const pid = r.projectId || '';
          if (!byDate[date][pid]) byDate[date][pid] = { projId: pid, projName: (proj || {}).name || '?', punches: [] };
          byDate[date][pid].punches.push(punchOut);
        });
        ...
        res.status(200).json({ weekStart: wk, days });
        return;
      }

      if (action === 'submitApproval') {
        // Approuve/rejette le poinçon d'un COLLÈGUE — REMPLACE directement le statut, exactement
        // comme les boutons ✔/✘ du portail Admin (resource 'punchesAdmin' ci-dessous, mêmes
        // champs status/approvedByName/rejectionComment). Aucune approbation Admin distincte
        // n'est requise ensuite.
        if (!me.isApprover) {
          res.status(403).json({ error: "Ce compte n'est pas autorisé à approuver des feuilles de temps." });
          return;
        }
        const { punchId, decision, comment } = req.body || {};
        if (!punchId || !['approuve', 'rejete'].includes(decision)) {
        ...
        const [punch] = await db.select().from(schema.punches).where(and(eq(schema.punches.id, punchId), eq(schema.punches.tenantId, tenantId)));
        if (!punch) { res.status(404).json({ error: 'Poinçon introuvable.' }); return; }
        if (punch.employeeId === employeeId) { res.status(403).json({ error: "Impossible d'approuver sa propre feuille de temps." }); return; }
        if (punch.status === 'exporte') { res.status(403).json({ error: 'Ce poinçon a déjà été exporté à la paie et ne peut plus être modifié.' }); return; }
        if (decision === 'approuve' && punch.status === OPEN_PUNCH_STATUS) {
        ...
        const date = torontoDateKey(new Date(punch.clockIn));
        const myDayRows = await db.select().from(schema.punches).where(and(eq(schema.punches.tenantId, tenantId), eq(schema.punches.employeeId, employeeId)));
        const ownMatch = myDayRows.some(r => r.clockIn && torontoDateKey(new Date(r.clockIn)) === date && r.projectId === punch.projectId);
        if (!ownMatch) {
        ...
        await db.update(schema.punches).set({
          status: decision === 'approuve' ? 'approuve' : 'rejete',
          approvedByName: me.fullName || '',
          rejectionComment: decision === 'rejete' ? String(comment).trim() : null,
        }).where(eq(schema.punches.id, punchId));
        res.status(200).json({ ok: true });
        return;
      }
    ...
    if (resource === 'punchesAdmin') {
      ...
      async function weekPunchesWithNames(weekStart) {
        ...
        const projMap = Object.fromEntries(projects.map(p => [p.id, p]));
        // Calcul PAR JOUR (voir computeDayGroup) — c'est cette fonction qui alimente directement
        // l'export de paie (exportPayroll, plus bas), d'où l'importance que l'Ajusté reflète bien
        // le segment "biggest" de chaque journée plutôt que chaque poinçon isolément.
        const { hoursByPunchId: hoursWeek } = computeDayAdjustedHours(weekRows, pauseConfig);
        return weekRows
          .sort((a, b) => new Date(a.clockIn) - new Date(b.clockIn))
          .map(r => {
            const proj = projMap[r.projectId] || {};
            return {
              ...r,
              ...(hoursWeek.get(r.id) || { brutH: null, ajusteH: null }),
      ...
      if (action === 'editPunch') {
        ...
        if ((Number(nextOvertime15) || 0) + (Number(nextOvertime2) || 0) > 0) {
          ...
          // Calcul PROJETÉ au niveau de la JOURNÉE (voir computeDayGroup) : on simule ce poinçon
          // avec ses valeurs après patch, aux côtés de ses éventuels segments frères de la même
          // journée (changement de chantier), pour savoir s'il serait "biggest" et quel serait son
          // Ajusté réel une fois enregistré — même principe que saveDayGroup() (admin.html, EPD).
          const projectedDateKey = torontoDateKey(new Date(nextClockIn));
          const siblingRowsEdit = await db.select().from(schema.punches)
            .where(and(eq(schema.punches.tenantId, tenantId), eq(schema.punches.employeeId, punch.employeeId)));
          const sameDaySiblingsEdit = siblingRowsEdit.filter(r => r.id !== id && r.clockIn && torontoDateKey(new Date(r.clockIn)) === projectedDateKey);
          const projectedPunch = {
            id: punch.id, employeeId: punch.employeeId,
            clockIn: nextClockIn, clockOut: nextClockOut,
            breakMorning: nextBreakMorning, breakLunch: nextBreakLunch, breakAfternoon: nextBreakAfternoon,
          };
          const { hoursByPunchId: projectedHours } = computeDayAdjustedHours([...sameDaySiblingsEdit, projectedPunch], pauseConfig);
          const ajusteH = (projectedHours.get(id) || {}).ajusteH || 0;
        ...
        const dateKeyEditFinal = torontoDateKey(new Date(row.clockIn));
        const dayRowsEditFinal = await db.select().from(schema.punches)
          .where(and(eq(schema.punches.tenantId, tenantId), eq(schema.punches.employeeId, row.employeeId)));
        const sameDayEditFinal = dayRowsEditFinal.filter(r => r.clockIn && torontoDateKey(new Date(r.clockIn)) === dateKeyEditFinal);
        const { hoursByPunchId: hoursEditFinal } = computeDayAdjustedHours(sameDayEditFinal, pauseConfig);
        res.status(200).json({ ok: true, punch: row, ...(hoursEditFinal.get(row.id) || { brutH: null, ajusteH: null }) });
        return;
      }
      ...
      if (action === 'approve') {
        ...
        // Nom de qui approuve — même champ que l'Approbateur mobile (voir resource 'punches' /
        // action 'submitApproval' ci-dessous) : le portail Admin et le mobile ÉCRASENT le même
        // statut, aucune approbation distincte n'est requise ensuite.
        const approverName = await getCallerDisplayName(ctx);
        await db.update(schema.punches).set({ status: 'approuve', approvedByName: approverName, rejectionComment: null }).where(eq(schema.punches.id, id));
        res.status(200).json({ ok: true });
        return;
      }
      ...
      if (action === 'reject') {
        ...
        if (!id) { res.status(400).json({ error: 'id est requis.' }); return; }
        if (!(comment || '').trim()) { res.status(400).json({ error: 'Un commentaire est requis pour rejeter une feuille de temps.' }); return; }
        const [punch] = await db.select().from(schema.punches).where(and(eq(schema.punches.id, id), eq(schema.punches.tenantId, tenantId)));
        if (!punch) { res.status(404).json({ error: 'Poinçon introuvable.' }); return; }
        ...
        if (punch.status === 'exporte') { res.status(400).json({ error: 'Ce poinçon a déjà été exporté.' }); return; }
        const approverName = await getCallerDisplayName(ctx);
        await db.update(schema.punches).set({ status: 'rejete', approvedByName: approverName, rejectionComment: String(comment).trim() }).where(eq(schema.punches.id, id));
        res.status(200).json({ ok: true });
        return;
      }
      ...
      if (action === 'restore') {
        ...
        await db.update(schema.punches).set({ status: 'ferme', approvedByName: null, rejectionComment: null }).where(eq(schema.punches.id, id));
        res.status(200).json({ ok: true });
        return;
      }
    ...
    // ===================== Confidentialité (Loi 25) — fusionné depuis l'ancien api/privacy.js
    ...
    if (resource === 'settings' || resource === 'incidents' || resource === 'dataRequests') {
      const isAdminRole = ctx.role === 'admin';
      const isAdminOrReadOnly = isAdminRole || ctx.role === 'lecture-seule';

      if (resource === 'settings') {
        if (!isAdminOrReadOnly) { res.status(403).json({ error: 'Accès réservé aux administrateurs de votre entreprise.' }); return; }
        if (action === 'get') {
      ...
      if (resource === 'incidents') {
        ...
        if (action === 'create') {
          ...
          const [row] = await db.insert(schema.privacyIncidents).values({
            ...
            createdByLabel: (ctx.claims && (ctx.claims.name || ctx.claims.email)) || ctx.callerId || null,
          }).returning();
      ...
      if (resource === 'dataRequests') {
        if (action === 'create') {
          // Ouvert aux employés (employeeRef forcé à leur propre id) ET aux admins/lecture-seule.
          let employeeRef = (req.body || {}).employeeRef || null;
          if (ctx.role === 'employee') {
            if (!ctx.claims.employeeItemId) { res.status(403).json({ error: "Ce compte n'a pas accès." }); return; }
            employeeRef = String(ctx.claims.employeeItemId);
