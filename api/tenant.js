  ...
  try {
    ...
    if (resource === 'config') {
      ...
      if (action === 'updateSettings') {
        const { primesEnabled, payrollSoftware, companyName, companyAddress, companyLogo, companyCodeAvantage } = req.body || {};
        const patch = {
    ...
    if (resource === 'punchesAdmin') {
      ...
      if (action === 'manualAdd') {
        const { employeeId, projectId, date, clockIn, clockOut, tache } = req.body || {};
      ...
      if (action === 'editPunch') {
        ...
        if ((Number(nextOvertime15) || 0) + (Number(nextOvertime2) || 0) > 0) {
          ...
          const projected = computePunchHours({ clockIn: nextClockIn, clockOut: nextClockOut, breakMorning: nextBreakMorning, breakLunch: nextBreakLunch, breakAfternoon: nextBreakAfternoon });
