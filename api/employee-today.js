...
// jeton de session employé valide (voir api/punch.js action 'login' et
// api/_auth/employeeAuth.js) contenant { role: 'employee', employeeItemId: '<id
// de l'item Monday employé>' } — authentification maison téléphone + mot de
// passe, Clerk n'est plus utilisé pour ce fichier.

const { verifyEmployeeToken } = require('./_auth/employeeAuth');
...
// Coché = affiche le bouton mobile "Approbation" (voir api/punch.js actions listForApproval/
// submitApproval, et admin.html/Comptes employés pour la case à cocher correspondante).
...
module.exports = async function handler(req, res) {
  ...
  let claims;
  try {
    claims = verifyEmployeeToken(bearerToken);
  } catch (err) {
  ...
  const query = `
    query($planningBoard: ID!, $week: [String]!, $projectsBoard: ID!, $stageCol: ID!, $stageVal: CompareValue!, $distBoard: ID!, $distCol: String!, $empId: [String]!, $empIds: [ID!], $metBoard: [ID!], $tachBoard: [ID!], $poinconsBoard: ID!, $pDateCol: String!, $pDate: [String]!) {
      ...
      moi: items(ids: $empIds) {
        id column_values(ids: ["${EMP_TITRE_COL}", "${EMP_APPROBATEUR_COL}"]) { id text }
      }
  ...
  try {
    ...
    // Voir EMP_APPROBATEUR_COL — affiche le bouton mobile "Approbation" (api/punch.js).
    const moiApprobateurCv = moiItem ? (moiItem.column_values || []).find(c => c.id === EMP_APPROBATEUR_COL) : null;
    const isApprobateur = (moiApprobateurCv && moiApprobateurCv.text) === 'v';
    ...
    let openPunch = null;
    // Statut (Début/Fin renseignés ou non) de TOUS les poinçons de l'employé aujourd'hui, pas
    // seulement celui encore ouvert — utilisé par le mobile pour détecter qu'un poinçon qu'il
    // croit "actif" en mémoire (localStorage) a en fait été complété entre-temps par l'admin
    // (ajout manuel du Fin ou édition directe dans admin.html), afin de sortir automatiquement
    // de l'état "En poste" sans que l'employé n'ait à cliquer "Terminer" lui-même.
    const myPunchesToday = [];
    for (const it of poinconItems) {
      ...
      myPunchesToday.push({ itemId: it.id, hasDebut, hasFin });
      if (!hasDebut || hasFin || openPunch) continue; // pas ouvert, ou un poinçon ouvert déjà trouvé
    ...
    res.status(200).json({
      ...
      kmSuggested, mySchedule: mySchedule2, titre, taches, openPunch, myPunchesToday, isApprobateur
    });
  } catch (err) {
